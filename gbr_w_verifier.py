# -*- coding: utf-8 -*-
"""GBR_w_verifier.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1AgCQEABwwrIIFYUchhgbsb1PQaBm0Uuq
...
Before running the code, first check if SHAP is already in library. If not, install using given code, similar applies to *os*. SHAP is an interpretability tool that will help us to see the contribution of each feature towards machine learning predictions.
"""
...
"""- Paths will be constructed according to the zone and name.
  ...
  An example would be:
  > *Enter home directory where the datasets are located:* **/content/airmoss/**
"""
...
"""- Input the name of the testing dataset, this will be used to construct the full path.
  ...
  An example would be:
  > *Enter the name of the testing dataset:* **A4_400m_processed__test.csv**
...
"""- Input the name of the training dataset, this will be used to construct the full path.
  ...
  An example would be:
  > *Enter the name of the training dataset:* **A4_400m_processed__train.csv**
...
"""- The next step is to create a variable that holds date and verifier columns:
  ...
  - The **ver** variable was created to hold the in situ data (Verifier), which can also be referred to as validation data. We created this variable to hold the validation data and place it in the output file so that we can compare the results in the final output. However, since **ver** is not used as a predictor, it will be removed from **test_data**.
    ...
    > **Note:** If you have two verifiers, you must include the second one in your variables and create another line of code to drop that verfier column. Use the following code to drop that second verifier. Copy the code, change the *Verifier* to the name of your second verifier and paste it in the next section of code.
```
